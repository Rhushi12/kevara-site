...
async function deleteDefinition(type: string) {
    console.log(`Deleting Definition: ${type}...`);
  ...
  `;
    ...
    if (!definitionId) {
        console.error(`❌ Definition ${type} NOT FOUND`);
        return;
    }
    console.log(`Found Definition ID for ${type}:`, definitionId);
  ...
  `;
    ...
    if (result.metaobjectDefinitionDelete.userErrors.length > 0) {
        console.error(`❌ Failed to delete definition ${type}:`, result.metaobjectDefinitionDelete.userErrors);
    } else {
        console.log(`✅ Definition ${type} Deleted Successfully!`);
    }
}

async function main() {
    await deleteDefinition("shop_essential_tab");
    await deleteDefinition("lookbook_section");
    await deleteDefinition("collection_grid_item");
}
